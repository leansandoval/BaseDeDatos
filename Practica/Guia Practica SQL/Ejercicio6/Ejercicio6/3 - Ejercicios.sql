...
CREATE OR ALTER VIEW vCantidadDeVuelosEsteAñoPorPasajeros AS
(
	SELECT ip.Documento, COUNT(*) [Cantidad de vuelos en este año]
...
SELECT v.Documento
FROM vCantidadDeVuelosEsteAñoPorPasajeros AS v
WHERE v.[Cantidad de vuelos en este año] = (SELECT MAX([Cantidad de vuelos en este año]) 
											FROM vCantidadDeVuelosEsteAñoPorPasajeros)
...
-- 11. Hallar los pasajeros que han realizado una cantidad de vuelos dentro del 10% en más o en menos del promedio
-- de vuelos de todos los pasajeros de la compañía.
